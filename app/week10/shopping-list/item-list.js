...
export default function ItemList({itemList, onItemDeleted , sendIngredient}) {
    const [sortCriteria, setSortCriteria] = useState('name');

    let workingList = itemList;

    const handleIngredient = (name) => {sendIngredient(name);};

    const handleDelete = (itemId,userId) => {onItemDeleted(itemId,userId);};
